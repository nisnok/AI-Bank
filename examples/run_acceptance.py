...
from acceptance.bundle import EvidenceRunWriter, stage
...
def main(args):
    if args.evidence_root.is_absolute():
        args.evidence_root=args.evidence_root.relative_to(Path.cwd())
    bundle=EvidenceRunWriter(args.evidence_root, kind="acceptance")
    try:
        return execute_acceptance(args, bundle)
    except BaseException:
        bundle.abort()
        raise


def execute_acceptance(args, bundle):
    root=bundle.directory
    env=dict(os.environ)
    env['AI_BANK_EVIDENCE_MANAGED']='1'
    env['AI_BANK_EVIDENCE_RUN']=str(root)
    ...
    def run(name,command,*,live=False,browser=False,timeout=300):
        ...
        child_env['AI_BANK_EVIDENCE_MANAGED']='1'
        child_env['AI_BANK_EVIDENCE_RUN']=str(root)
        ...
        checks[name]['command']=' '.join([('pyright' if Path(arg).name=='pyright' else 'python') if i==0 else arg for i,arg in enumerate(command)])
    ...
    run('evaluation',[python,'examples/run_evaluation.py','--evidence-root',str(stage(root,'evals'))])
    row('Automated evaluation','evaluation','All 13 scenario expectations match; classified failures',stage(root,'evals'))
    run('tenants',[python,'examples/replay_tenant.py','--all','--evidence-root',str(stage(root,'reuse'))])
    row('H. Multi-tenant reuse','tenants','Bank A and B succeed with unchanged canonical artifact and zero models',stage(root,'reuse'))
    row('I. Safe and ambiguous drift','tenants','Unique fallback succeeds; ambiguity stops before search',stage(root,'reuse'))
    row('F. Ambiguity','tenants','HUMAN_REQUIRED; ambiguous search submissions = 0',stage(root,'reuse'))
    run('health',[python,'examples/run_reliability_eval.py','--evidence-root',str(stage(root,'health'))])
    row('J. Capability health','health','Bank A HEALTHY; successful Bank B DEGRADED from telemetry',stage(root,'health'))
    run('handoff',[python,'examples/run_handoff.py','--scripted','--evidence-root',str(stage(root,'handoff'))])
    row('G. Same-session handoff','handoff','Two mediated actions; no automation during HUMAN; one account created',stage(root,'handoff'))
    run('discovery_handoff',[python,'examples/accept_discovery_handoff.py','--evidence-root',str(stage(root,'discovery-handoff'))])
    row('Discovery-time same-session handoff','discovery_handoff',
        'Retained page; HUMAN ownership; audited action; verified fresh decision; SUCCESS',stage(root,'discovery-handoff'))
    run('recovery',[python,'examples/accept_recovery.py','--evidence-root',str(stage(root,'recovery'))])
    row('E. Bounded recovery','recovery','One safe wait retry; one search submission; RECOVERABLE_ERROR',stage(root,'recovery'))
    if args.live_discovery:
        run('live_discovery',[python,'examples/accept_discovery.py','--evidence-root',str(stage(root,'live'))],live=True)
        path=stage(root,'live')/'report.json'
    ...
    for key,claim,expected in (
        ...
        rows.append({'claim':claim,'command':checks.get('live_discovery',{}).get('command',
            'python examples/run_acceptance.py --live-discovery'),'expected':expected,
            'evidence':item.get('evidence',str(stage(root,'live')/'report.json')),'status':item['status'],
            'reason':item.get('reason')})
    row('Security browser checks / selective redaction','browser_suite',
        'Pixel masks; visible headings; full-mask fallback; blocked origins; bounded injection',root/'evaluation/checks.json')
    try:
        independent=verify(root)
    except Exception:
        independent={'status':'FAIL','reason':'SAVED_EVIDENCE_ASSERTION_FAILED'}
    (root/'evaluation/independent-checks.json').write_text(json.dumps(independent,indent=2)+'\n')
    rows.append({'claim':'Independent saved-evidence verification','command':'acceptance.verify.verify',
                 ...
                 'evidence':str(root/'evaluation/independent-checks.json'),'status':independent['status']})
    (root/'evaluation/checks.json').write_text(json.dumps(checks,indent=2)+'\n')
    ...
    (root/'evaluation/source-privacy-scan.json').write_text(json.dumps(privacy,indent=2)+'\n')
    rows.append({'claim':'Evidence secret/PII canary scan','command':'acceptance.privacy.scan',
                 ...
                 'evidence':str(root/'evaluation/source-privacy-scan.json'),'status':privacy['status']})
    ...
    (root/'evaluation/source-index.md').write_text('\n'.join(lines)+'\n')
    summary=bundle.finish(checks=checks, claims=rows, independent=independent)
    print(f"Acceptance: {root}/README.md ({summary['overall_status']})",flush=True)
