...
Actual runs write existing evidence bundles under `evidence/<run_id>/`: metadata, JSONL events, final result, and masked screenshots. Locator attempts, successful strategy indices, quality, conditions, retries, and policy decisions remain inspectable. Runtime values, bound expectations, and exception messages remain excluded. Screenshots are fully masked, so they provide timing/presence evidence rather than readable UI diagnostics. No evidence is fabricated or committed.
